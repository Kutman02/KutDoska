...
import type { Chat, Message, AllMessages } from '../types/page.types';
...
const Chats: React.FC = () => {
  ...
  const [activeChatId, setActiveChatId] = useState<number>(initialChatList[0].id);
  ...
  const messages: Message[] = allMessages[activeChatId] || [];
  ...
  return (
    <div className="relative min-h-screen w-full">
      {/* Баннер режима разработки */}
      <div className="w-full bg-yellow-400 border-b-2 border-yellow-600 px-4 py-3 text-center z-50 relative">
        <div className="max-w-7xl mx-auto flex items-center justify-center gap-2">
          <span className="text-yellow-900 font-bold text-sm md:text-base">
            ⚠️ РАЗДЕЛ В РАЗРАБОТКЕ - Демонстрационный режим
          </span>
        </div>
      </div>
      
      {/* Общий контейнер */}
      <div className="flex h-[calc(100vh-3.5rem)] w-full max-w-7xl mx-auto shadow-xl border border-gray-200 rounded-lg overflow-hidden relative opacity-90">
        {/* Водяной знак "В РАЗРАБОТКЕ" */}
        <div className="absolute inset-0 pointer-events-none z-10 flex items-center justify-center">
          <div className="transform -rotate-45 text-gray-300 dark:text-gray-600 text-6xl md:text-8xl font-bold opacity-20 select-none">
            В РАЗРАБОТКЕ
          </div>
        </div>
      ...
      <aside 
        ...
        className={`w-full md:w-80 flex--0 flex flex-col border-r border-gray-200 bg-white ${
          isChatListVisible ? 'flex' : 'hidden md:flex'
        }`}
      >
        
        <header className="p-4 border-b border-gray-200 relative z-20">
          <input 
            type="text" 
            placeholder="Поиск по чатам..." 
            disabled
            className="w-full p-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-100 cursor-not-allowed opacity-60" 
          />
        </header>

        <div className="grow overflow-y-auto relative z-20">
          {initialChatList.map((chat: Chat) => (
            <div 
              key={chat.id} 
              onClick={() => handleChatSelect(chat.id)} // <--- Обработчик клика
              className={`flex items-center p-3 border-b border-gray-100 cursor-pointer transition duration-150 ease-in-out opacity-75 ${
                chat.id === activeChatId // Проверяем активный ID
                  ? 'bg-blue-50 border-l-4 border-blue-500' 
                  : 'hover:bg-gray-50'
              }`}
            >
              <div className="shrink-0 w-10 h-10 bg-blue-500 text-white flex items-center justify-center rounded-full font-bold mr-3">
                {chat.name[0]}
              </div>
      ...
      >
        ...
        {/* Заголовок чата */}
        <header className="p-4 bg-white border-b border-gray-200 flex items-center shrink-0 relative z-20 opacity-75">
        ...
        {/* Область сообщений */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-200 relative z-20 opacity-75">
        ...
        <footer className="p-4 bg-white border-t border-gray-200 flex items-center shrink-0 relative z-20 opacity-75">
          <button disabled className="p-2 text-gray-500 hover:text-blue-600 transition text-xl cursor-not-allowed opacity-50">
            📎
          </button>
          <input 
            type="text" 
            ...
            disabled
            className="grow mx-3 p-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-100 cursor-not-allowed opacity-60" 
          />
          <button disabled className="bg-blue-500 text-white p-3 rounded-full hover:bg-blue-600 transition text-lg cursor-not-allowed opacity-50">
            🚀
          </button>
        </footer>
      </main>
